    ...
    private boolean run(InetSocketAddress server) throws Exception {
        ...
        try {
            ...
            while (true) {
                ...
                if (length > 0) {
//                    byte[] encryptedPacket= encryptDecrypt(packet.array());
//
//                    length=encryptedPacket.length+1;
//
//                    // Control messages always start with zero.
//                    packet.rewind();
//                    packet.put((byte) 1).put(encryptedPacket).flip();
//                    //                    // Write the outgoing packet to the tunnel.
                    ...
                    if (timer < 1) {
                        timer = 1;
                        Log.v(TAG, "We are receiving data. ");
                    }

                    if (timer < 1)
                    {
                        Log.v(TAG, "We are sending data. ");
                    }
                    else if (timer == 1)
                    {
                        Log.v(TAG, "We are receiving data. ");
...
//                        packet.get(data, 1, length);
//                        byte[] decryptedPacket= encryptDecrypt(data);
//                        length=decryptedPacket.length;
//
