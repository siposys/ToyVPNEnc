...
static void *read_send(void *ptr) {
    ...
    do {
		...
		if(int(packet[0])!=0){
			//char* decodedPacket=XORCipher((packet+1), (char*)"secret", length-1, strlen((char*)"secret"));
			//int decodedLength=strlen(decodedPacket);
			//if ((length = send(socket, decodedPacket, decodedLength, MSG_NOSIGNAL)) == -1) {
...
static void *recv_write(void *ptr) {
    ...
    do {
        ...
        if (packet[0] != 0) {
			char* encodedPacket=XORCipher((packet+1), (char*)"secret", length-1, strlen((char*)"secret"));
			int encodedLength=strlen(encodedPacket);
            if ((length = write(interface, encodedPacket, encodedLength)) == -1) {
